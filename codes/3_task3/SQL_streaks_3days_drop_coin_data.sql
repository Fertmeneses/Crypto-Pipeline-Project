...
-- psql -h 127.0.0.1 -U postgres -d postgres -v init_date='2021-02-01' -v final_date='2021-06-30' -f SQL_streaks_3days_drop_coin_data.sql
-- If you want to omit any of these parameters, you can choose to set them as an empty string '', for example:
-- psql -h 127.0.0.1 -U postgres -d postgres -v init_date='' -v final_date='' -f SQL_streaks_3days_drop_coin_data.sql
...
price_usd_changes AS (
    ...
    (json->'market_data'->'market_cap'->>'usd')::numeric AS market_cap_usd
    FROM filtered_data
),
-- Flag drop days --
drop_flags AS (
